...
func (s *MockService) nodeProbe(ctx context.Context, cluster *IsilonClusterConfig) error {
	args := s.Called(ctx, cluster)
	return args.Error(0)
}

func (s *MockService) testConnectivityAndUpdateStatus(ctx context.Context, cluster *IsilonClusterConfig, timeout time.Duration) {
	defer func() {
		if err := recover(); err != nil {
			s.Called("panic occurred", err, cluster) // Capture the panic
			// Restart the function after panic (simulating async behavior)
			go s.testConnectivityAndUpdateStatus(ctx, cluster, timeout)
		}
	}()
	// Mock test logic here
	if err := s.nodeProbe(ctx, cluster); err != nil {
		// Panic on failure (to simulate recovery behavior)
		panic("nodeProbe failed")
	}
}

func Test_ConnectivityAndUpdateStatus(t *testing.T) {
	// Create the mock logger and mock service
	mockLogger := new(MockLogger)
	mockService := new(MockService)

	// Set up mock expectations
	mockService.On("nodeProbe", mock.Anything, mock.Anything).Return(fmt.Errorf("test error"))
	mockLogger.On("Errorf", mock.Anything, mock.Anything, mock.Anything).Return()

	// Test the panic recovery behavior
	cluster := &IsilonClusterConfig{
		ClusterName: "TestCluster",
	}

	// Call the function and expect it to panic and recover
	go mockService.testConnectivityAndUpdateStatus(context.Background(), cluster, 5*time.Second)

	// Give some time for the goroutine to execute
	time.Sleep(1 * time.Second)

	// Ensure panic recovery was called
